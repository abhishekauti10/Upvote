...
public class UserActivity {
	...
	public void deleteQuestion(UserActivity userActivity, User user) throws NumberFormatException, IOException {
		...
		User userA = question.getUser(); //CREATING NEW OBJECT TO STORE userA = question.getUser(), userA posted the question
		...
		if (user.getUserRole() == UserRole.ADMIN) {  	// the user is an admin
			...
			if(userA.getUserRole() == UserRole.USER || user.getUsername() == userA.getUsername()) { //IF PERSON WHO POSTED THE QUESTION IS A USER OR QUESTION WAS POSTED BY THE MODERATOR ITSELF
			...
			if(userA.getUsername() == user.getUsername()){ // IF PERSON WHO POSTED THE QUESTION IS SAME AS THE PERSON MAKING DELETION REQUEST
				questionService.deleteQuestion(question);
				System.out.println("Question Number " + question.getId() + " Deleted Successfully");
			} else System.out.println("You are not authorised to delete this question");
		}
